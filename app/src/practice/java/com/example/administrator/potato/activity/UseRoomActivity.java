...
public class UseRoomActivity extends BaseActivity {
    ...
    protected void initData() {
        //进入时自动查询一次
        BaseDataBaseUtil.getDataBaseUtil(getApplicationContext()).studentDao().getAllStudents()
            ...
            case R.id.buttonInsert:
                Single.fromCallable(new Callable<List<Long>>() {
                    ...
                    public List<Long> call() throws Exception {
                        ...
                        studentModel.setStudentName("小花");
                        studentModel.setStudentAge(18);
                        studentModel.setStudentAddress("美国");
                        //这里返回的 其实是插入之后的数据的行id 因此需要使用一个list来存储
                        return BaseDataBaseUtil.getDataBaseUtil(getApplicationContext()).studentDao().insertAll(studentModel);
                    }
                }).subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                ...
                Single.fromCallable(new Callable<Integer>() {
                    ...
                    public Integer call() throws Exception {
                        ...
                        if (studentModelList.size() > 0) {
                            return BaseDataBaseUtil.getDataBaseUtil(getApplicationContext()).studentDao().delete(studentModelList.get(0));
                        } else {
                            return -1;
                        }
                    }
            ...
            case R.id.buttonModify:
                Single.fromCallable(new Callable<Integer>() {
                    ...
                    public Integer call() throws Exception {
                        //修改前先判断数据库是否有值
                        if (studentModelList.size() > 0) {
                            studentModelList.get(0).setStudentName("哗哗哗");
                            return BaseDataBaseUtil.getDataBaseUtil(getApplicationContext()).studentDao().update(studentModelList.get(0));
                        } else {
                            return -1;
                        }
                    }
